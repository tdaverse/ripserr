...
To imitate the model from Lo and Park with available data, we will predict the number of ZIKV cases confirmed in each state using linear regression.
...
In addition to these state-level attributes, we will utilize spatial information on _A. aegypti_ occurrences.[^aegypti]
...
Also, we load the `zikaPredict` dataset to obtain human population density, average temperature, precipitation level, and the number of cases in each state.
...
We merge `zikaModel` with `zikaPredict`, matched by state code.
We have now loaded all potential predictors to our model.
The following code prints out first few lines of the `zikaModel` to demonstrate how states and their variables lie within this data frame.
