...
sagAstar_img <- "https://upload.wikimedia.org/wikipedia/commons/thumb/9/96/EHT_Saggitarius_A_black_hole.tif/lossy-page1-240px-EHT_Saggitarius_A_black_hole.tif.jpg"
# read image and convert it to grayscale
powehi_img <- image_read(powehi_img) |> image_convert(colorspace = "gray")
sagAstar_img <- image_read(sagAstar_img) |> image_convert(colorspace = "gray")
# make the image into a bitmap matrix
powehi_mat <- as.numeric(image_data(powehi_img)) |> drop() |> t()
powehi <- powehi_mat[, ncol(powehi_mat):1]

sagAstar_mat <- as.numeric(image_data(sagA_img)) |> drop() |> t()
sagAstar <- sagA_mat[, ncol(sagA_mat):1]
...
save(sagAstar, file = "sagAstar.rda")
