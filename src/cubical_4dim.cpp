...
public:
  ...
  BirthdayIndex4();
  BirthdayIndex4(const BirthdayIndex4& b);
  BirthdayIndex4(double _b, int _i, int _d);
  void copyBirthdayIndex(BirthdayIndex4 v);
  double getBirthday();
  long getIndex();
  int getDimension();
  void print();
...
BirthdayIndex4::BirthdayIndex4(){
  birthday = 0;
  index = -1;
  dim = 1;
}

BirthdayIndex4::BirthdayIndex4(const BirthdayIndex4& b){
  birthday = b.birthday;
  index = b.index;
  dim = b.dim;
}

BirthdayIndex4::BirthdayIndex4(double _b, int _i, int _d){
  birthday = _b;
  index = _i;
  dim = _d;
}

void BirthdayIndex4::copyBirthdayIndex(BirthdayIndex4 v){
  birthday = v.birthday;
  index = v.index;
  dim = v.dim;
}

double BirthdayIndex4::getBirthday(){
  return birthday;
}

long BirthdayIndex4::getIndex(){
  return index;
}
...
bool BirthdayIndex4InverseComparator::operator()(const BirthdayIndex4& o1, const BirthdayIndex4& o2) const{
  if(o1.birthday == o2.birthday){
    if(o1.index < o2.index){
      return false;
    } else {
...
public:
  ...
  WritePairs4(int64_t _dim, double _birth, double _death);
  int64_t getDimension();
  double getBirth();
  double getDeath();
};

WritePairs4::WritePairs4(int64_t _dim, double _birth, double _death){
  dim = _dim;
  birth = _birth;
  death = _death;
}

int64_t WritePairs4::getDimension(){
  return dim;
}

double WritePairs4::getBirth(){
  return birth;
}

double WritePairs4::getDeath(){
  return death;
}
      ...
      for (int y = ay; y > 0; --y) {
        for (int x = ax; x > 0; --x) {
          ...
          if (birthday != _dcg -> threshold) {
            columns_to_reduce.push_back(BirthdayIndex4(birthday, index, 0));
          }
        }
      }
    }
...
public:
  ...
  void setSimplexCoboundaryEnumerator(BirthdayIndex4 _s, DenseCubicalGrids* _dcg);
  bool hasNextCoface() ;
...
void SimplexCoboundaryEnumerator::setSimplexCoboundaryEnumerator(BirthdayIndex4 _s, DenseCubicalGrids* _dcg) {
  simplex = _s;
  dcg = _dcg;
  dim = simplex.dim;
...
public:
  JointPairs(DenseCubicalGrids* _dcg, ColumnsToReduce* _ctr, vector<WritePairs4> &_wp, const bool _print);
...
JointPairs::JointPairs(DenseCubicalGrids* _dcg, ColumnsToReduce* _ctr, vector<WritePairs4> &_wp, const bool _print){
  dcg = _dcg;
  ax = dcg -> ax;
  ay = dcg -> ay;
  az = dcg -> az;
        ...
        for(int w = 1; w <= aw; ++w){
          for(int type = 0; type < 4; ++type){ // change
            ...
            if(birthday < dcg -> threshold){
              dim1_simplex_list.push_back(BirthdayIndex4(birthday, index, 1));
            }
          }
        }
      }
...
public:
  ...
  ComputePairs(DenseCubicalGrids* _dcg, ColumnsToReduce* _ctr, vector<WritePairs4> &_wp, const bool _print);
...
ComputePairs::ComputePairs(DenseCubicalGrids* _dcg, ColumnsToReduce* _ctr, vector<WritePairs4> &_wp, const bool _print){
  dcg = _dcg;
  ctr = _ctr;
  dim = _ctr -> dim;
  wp = &_wp;
