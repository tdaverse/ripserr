...
`Ripser` was first released in 2016, but it went through several upgrades before being described in the celebrated journal article [@Bauer2021].
...
{ripserr} is becoming an underpin of the TDAverse: It powers the computation of PH in the {recipes} extension [{tdarec}](https://cran.r-project.org/package=tdarec) and the {ggplot2} extension [{ggtda}](https://github.com/tdaverse/ggtda), which bring PH into tidyverse and tidymodels workflows, and it couples with {phutil} and other tools in development to better interoperate within our own collection.
...
{ripserr} computes the birth and death radii of intervals ("features") in the Vietoris--Rips persistence module for a point cloud (finite metric space).
The results are returned as a `PHom` object, a light data frame wrapper with a concise print method.
For example, a large sample from a 2-sphere yields two dominant features, which encode the 0-dimensional connected component and the 2-dimensional spherical cell:

```{r phom}
# sample from a 2-sphere
x <- tdaunif::sample_sphere(n = 360, dim = 2)
head(x)
# compute & print persistent homology
( x_ph <- vietoris_rips(x, max_dim = 2) )
# inspect most-persistent features
x_ph |> 
  as.data.frame() |> 
  transform(persistence = abs(death - birth)) |> 
  sort_by(~ persistence) |> 
  tail()
```

{ripserr} accepts several input data structures, including the coordinate matrix above, but converts them to `dist` internally before passing to Ripser.
This allows more flexibility on the R side while simplifying the interface to C++.
The output constitutes one _persistence diagram_ for each dimension of feature (gap, loop, cell, or higher-order enclosure), and we refer to such data for one or more point clouds as _persistence data_.

The upgrade to {ripserr} v1.0.0[^v0.3.0] was straightforward, though not at all trivial:
Essentially, we plopped the current "main" version of `Ripser` into where the previous version had been, reconfiguring the {Rcpp} infrastructure along the way.[^forks]
This brought two immediate improvements: a modest increase in efficiency, and proper encodings of deaths at infinity, and we illustrate these below.
...
It also required some deeper engagement with the source: to ensure that outputs were properly collected and reformatted for return to R, to expose additional parameters between R and C++, and so on.
In several small ways, we tried to make this upgrade more faithful to the core tool and more familiar to users with some TDA background.

@fig-benchmark compares the runtimes of two versions of {ripserr} to compute degree-2 persistence on samples from 4 manifolds: the circle $\mathbb{S}^1$, the torus $\mathbb{T}^2$, the real projective plane $\mathbb{RP}^2$, and the orthogonal matrix group $O(3)$.[^ripsDiag]
(The first three were generated using {tdaunif}; the fourth was borrowed from Ripser.)
...
[^ripsDiag]: We tried to benchmark the same calculations using `TDA::ripsDiag(library = "Dionysus")`, the only library option for `dist` objects, but they failed to complete.

![Runtimes (in seconds) of 3 calculations each of 2-dimensional Vietoris--Rips persistent homology of samples of size 840 from 4 embedded manifolds.](benchmark-plot.jpg){#fig-benchmark}
...
The results with bounded threshold additionally include features whose births subceed the threshold but whose deaths exceed it. Note that the degree-1 component, not yet born at a radius of 800 miles, remains absent.
...
Strictly on the R side, recent changes to the development version on GitHub extend these tools to new data types and settings:

* An internal transformation of the array data allows `cubical()` to perform superlevel as well as sublevel set filtrations. This is controlled by the new `sublevel` parameter, which behaves like that of `TDA::gridDiag()`.
...
* Finally, {ripserr}'s `PHom` class will soon be deprecated in favor of {phutil}'s `persistence` class, as part of a slow harmonization across the TDAverse.

As ever, we welcome user feedback on every front!
And as before, we thank the R Consortium, especially for supporting the research assistants who performed much of this work.
